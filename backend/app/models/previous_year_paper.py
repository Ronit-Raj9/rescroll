...
class PreviousYearPaper:
    ...
    async def get_collection(cls):
        if not cls.collection:
            cls.collection = db.db.previous_year_papers
            # Create indexes
            await cls.collection.create_index("year")
            await cls.collection.create_index("subject")
            await cls.collection.create_index([("year", -1), ("subject", 1)])
    ...
    @classmethod
    async def find_many(cls, filter_dict: Dict[str, Any], skip: int = 0, limit: int = 10):
        collection = await cls.get_collection()
        cursor = collection.find(filter_dict).sort([("year", -1), ("subject", 1)]).skip(skip).limit(limit)
